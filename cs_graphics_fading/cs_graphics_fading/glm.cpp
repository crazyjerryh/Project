 ...
 */
static bool glmFirstPass(GLMmodel* model, FILE* file, std::string *errorString)
{
 ...
 */
bool glmReadOBJ(char* filename, GLMmodel *model, std::string *errorString)
{
    FILE*     file;
    ...
    if (!file) {
        fprintf(stderr, "glmReadOBJ() failed: can't open data file \"%s\".\n",
                filename);
		*errorString = "can't open data file";
    ...
    if(!glmFirstPass(model, file, errorString))
		return false;
...
GLvoid glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
        ...
        for (i = 0; i < group->numtriangles; i++) {
            ...
            GLfloat *temp = &model->vertices[3 * triangle->vindices[0]];
            qDebug() << "the point is (" << temp[0] << "," << temp[1] << "," << temp[2] <<"\n";
